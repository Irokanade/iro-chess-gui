...
void initialise_rook_attacks() {
	...
	for (Square sq = a1; sq <= h8; sq = square_inc(sq)) {
...
void initialise_squares_between() {
	Bitboard sqs;
	for (Square sq1 = a1; sq1 <= h8; sq1 = square_inc(sq1))
		for (Square sq2 = a1; sq2 <= h8; sq2 = square_inc(sq2)) {
...
void initialise_pseudo_legal() {
	...
	for (Square s = a1; s <= h8; s = square_inc(s)) {
