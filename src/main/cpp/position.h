...
void Position::play(const Move m) {
    side_to_play = color_invert(side_to_play);
    ++game_ply;
    ...
    case DOUBLE_PUSH:
        ...
        history[game_ply].epsq = square_add(m.from(), relative_dir<C>(NORTH));
        break;
    case OO:
        if (C == WHITE) {
    ...
    case EN_PASSANT:
        move_piece_quiet(m.from(), m.to());
        remove_piece(square_add(m.to(), relative_dir<C>(SOUTH)));
        break;
    case PR_KNIGHT:
        remove_piece(m.from());
...
void Position::undo(const Move m) {
    ...
    case EN_PASSANT:
        move_piece_quiet(m.to(), m.from());
        put_piece(make_piece(color_invert(C), PAWN), square_add(m.to(), relative_dir<C>(SOUTH)));
        break;
    case PR_KNIGHT:
    case PR_BISHOP:
    case PR_ROOK:
...
Move* Position::generate_legals(Move* list) {
    constexpr Color Them = color_invert(Us);
    ...
    while (b2) {
        s = pop_lsb(&b2);
        *list++ = Move(square_sub(s, relative_dir<Us>(NORTH)), s, QUIET);
    }

    while (b3) {
        s = pop_lsb(&b3);
        *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_NORTH)), s, DOUBLE_PUSH);
    }
    ...
    while (b2) {
        s = pop_lsb(&b2);
        *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_WEST)), s, CAPTURE);
    }

    while (b3) {
        s = pop_lsb(&b3);
        *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_EAST)), s, CAPTURE);
    }
    ...
    if (b1) {
        ...
        while (b2) {
            ...
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH)), s, PR_KNIGHT);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH)), s, PR_BISHOP);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH)), s, PR_ROOK);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH)), s, PR_QUEEN);
        }
        ...
        while (b2) {
            s = pop_lsb(&b2);
            //One move is added for each promotion piece
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_WEST)), s, PC_KNIGHT);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_WEST)), s, PC_BISHOP);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_WEST)), s, PC_ROOK);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_WEST)), s, PC_QUEEN);
        }

        while (b3) {
            s = pop_lsb(&b3);
            //One move is added for each promotion piece
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_EAST)), s, PC_KNIGHT);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_EAST)), s, PC_BISHOP);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_EAST)), s, PC_ROOK);
            *list++ = Move(square_sub(s, relative_dir<Us>(NORTH_EAST)), s, PC_QUEEN);
        }
    }
